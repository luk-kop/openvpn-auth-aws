...
make build-lambda           # Build Lambda Router zips (arm64 + amd64) in lambda-router/
make test                   # Unit tests: go test -v -short ./...
go test -v -short ./internal/auth/...  # Run tests for a single package
cd lambda-router && go test -v -short ./...  # Lambda Router tests (separate Go module)
...
OpenVPN auth daemon that orchestrates browser-based OIDC flows with AWS Cognito. The daemon connects to OpenVPN's Unix management socket, receives client events, and drives authentication through an ALB with a Cognito authenticate action.
...
- `internal/metrics/` — CloudWatch EMF metrics

**Entry points:** `cmd/openvpn-auth-daemon/` (main daemon), `cmd/mgmt-mock/` (OpenVPN simulator), `cmd/alb-mock/` (ALB + Cognito mock).

**Lambda Router** (`lambda-router/`): Separate Go module. ALB target for multi-instance mode — extracts instance IP from `/callback/{instance-id}/{proto}` path, validates IP against VPC CIDR, proxies request to the correct EC2 daemon. Case-insensitive OIDC header forwarding.

## Deployment Modes

- **Single-instance** (`multi_instance_mode=false`, default): Static ALB listener rules, EIP association, one server.
- **Multi-instance** (`multi_instance_mode=true`): ASG with NLB for OpenVPN traffic, Lambda Router manages ALB callback routing per instance.
- **Cost-saving** (`cost_saving_mode=true`): Deploys only Cognito + Secrets Manager (no ALB, EC2, EIP, Lambda). Useful for bootstrapping PKI.

## Terraform

Modules: `cognito`, `alb`, `nlb`, `vpn-server`, `lambda-router`. Root config in `terraform/`. OpenVPN version is pinned via `openvpn_version` variable (default `2.6.19`), flows through to cloud-config template and Docker build arg.

## Pre-commit Hooks

Configured in `.pre-commit-config.yaml`:
- Go: `golangci-lint run`, `go vet ./...`, `go test ./...`, `go mod tidy` (with diff check), `govulncheck ./...`
- Terraform: `terraform_fmt`, `terraform_docs`, `terraform_tflint`, `terraform_trivy`
- File hygiene: trailing whitespace, EOF, merge conflicts, AWS credential detection, private key detection
